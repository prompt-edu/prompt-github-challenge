...
export const Assessment = () => {
  ...
  const remainingAttempts = Math.max(
    (developerQuery.data?.maxAttempts ?? 0) -
      (developerQuery.data?.attempts ?? 0),
    0,
  );
      ...
      <CardContent>
        <div className="flex items-start space-x-2 mt-4">
          ...
          {participationError && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                An error occurred while fetching the assessment status. Please
                try again later.
              </AlertDescription>
            </Alert>
          )}
          ...
          {!passed && remainingAttempts === 0 && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                You did not successfully complete the technical challenge and
                have no remaining attempts.
              </AlertDescription>
            </Alert>
          )}
        </div>
            ...
            {remainingAttempts !== maxAttempts || assessmentTriggered ? (
              <Alert variant={!passed ? "destructive" : "default"}>
                <div className="flex items-center">
                  ...
                  <AlertTitle
                    className={`ml-2 ${passed ? "text-green-500" : ""}`}
                  >
                    {!passed ? "Error" : "Success"}
                  </AlertTitle>
                </div>
                <AlertDescription
                  className={`mt-1 ${passed ? "text-green-500" : ""}`}
                >
                  {!passed
        ...
        <div className="flex items-start space-x-2 mt-4">
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              !passed && setConfirmedOwnWork(checked as boolean)
            }
        ...
        <div className="flex items-start space-x-2 mt-4">
          ...
          >
            ...
            <span>
              {assessmentMutation.isPending ? "Testing..." : "Start Testing"}
            </span>
          </Button>
        </div>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription className="pt-2">
                Starting this test will decrease your remaining attempts by 1.
                You currently have <strong>{remainingAttempts}</strong> attempt
                {remainingAttempts !== 1 ? "s" : ""} remaining.
                <Alert variant="default" className="mt-4">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertTitle>Important</AlertTitle>
                  <AlertDescription>
                    Once you start the test, the attempt will be counted even if
                    you close the browser or encounter technical issues.
                  </AlertDescription>
                </Alert>
              </DialogDescription>
            </DialogHeader>
