...
const getChallengeStatus = (profile: DeveloperWithInfo | undefined): ChallengeStatus => {
  if (!profile) {
...
export const ResultsOverviewPage = () => {
  ...
  const developerProfilesByParticipationId = useMemo(
    () =>
      new Map((developerProfiles ?? []).map((profile) => [profile.courseParticipationID, profile])),
    [developerProfiles],
  )
  ...
  const extraColumns = useMemo<ExtraParticipantColumn<unknown>[]>(() => {
    const challengeStatusData = participations.map((participation) => ({
      ...
      value: getChallengeStatus(
        developerProfilesByParticipationId.get(participation.courseParticipationID),
      ),
    }))
    ...
    const passedAtData = participations.map((participation) => ({
      courseParticipationID: participation.courseParticipationID,
      value:
        developerProfilesByParticipationId.get(participation.courseParticipationID)?.passedAt ??
        null,
    }))
    ...
    const attemptsData = participations.map((participation) => ({
      courseParticipationID: participation.courseParticipationID,
      value:
        developerProfilesByParticipationId.get(participation.courseParticipationID)?.attempts ??
        null,
    }))
    ...
    return [
      {
        ...
        cell: ({ getValue }) => getChallengeStatusBadge(getValue() as ChallengeStatus),
        enableSorting: true,
    ...
    <div className='relative flex flex-col'>
      <div className='space-y-6'>
        ...
        <div className='text-sm text-muted-foreground'>
          {studentsPassedChallengeCount ?? 0} passed challenge | {studentsPassedCount} accepted
        </div>
        <CoursePhaseParticipationsTable
          phaseId={phaseId ?? ''}
